...
function wcifu_single_eng_download_button($arr, $doc_id, $file_size = '') {
	...
	if (! empty($arr['url_base'])) {
		...
		?> <div class="english-dl" data-doc-id="<?php echo $doc_id; ?>" ><span class="wcifu-button-title"><?php echo $arr['label']; ?></span>
			<a type="button" download="" data-fname="<?php echo $arr['filename']; ?>" data-folder="<?php echo $arr['url_base']; ?>" href="#" class="wcifu-download-link button" target="_blank" onmouseenter="setEngButtonUrl(this);">Download PDF</a>
			<?php
			if ($file_size) {
...
if (!function_exists('wcifu_translations_download_button')) {
	function wcifu_translations_download_button($arr, $doc_id, $file_size = '') {
		...
		if (! empty($arr)) {
			...
			?> <div class="lang-dl" data-doc-id="<?php echo $doc_id; ?>" ><span class="wcifu-button-title">Choose a translation: <select name="choose" data-id="<?php echo $data_id; ?>" class="inputLang" onchange="setLangButtonUrl(this);">
			...
			?> </select></span>
				<a type="button" class="wcifu-download-link button" download="" id="<?php echo $data_id; ?>" data-folder="<?php echo $id['url_base']; ?>" href="#" target="_blank">Download PDF</a>
			</div>
			<?php
			}
		else { ?>
